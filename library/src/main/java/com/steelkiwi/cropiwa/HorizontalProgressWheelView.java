...
public class HorizontalProgressWheelView extends View {
    ...
    private float mLastTouchedPosition, mTotalTouchedPosition;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mTotalTouchedPosition = event.getX();
                break;
            case MotionEvent.ACTION_UP:
                if (mScrollingListener != null) {
                    mScrollStarted = false;
                    float distance = event.getX() - mTotalTouchedPosition;
                    if(distance != 0){
                        mScrollingListener.onScrollEnd(distance);
                    }
                }
                break;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mProgressLinePaint.setColor(getResources().getColor(R.color.crop_color_progress_wheel_line));
        ...
        canvas.drawLine(mCanvasClipBounds.centerX(), mCanvasClipBounds.centerY() - mProgressLineHeight / 2.0f, mCanvasClipBounds.centerX(),
    ...
    private void init() {
        mMiddleLineColor = ContextCompat.getColor(getContext(), R.color.crop_color_middle_line);

        mProgressLineWidth = getContext().getResources().getDimensionPixelSize(R.dimen.crop_width_horizontal_wheel_progress_line);
        mProgressLineHeight = getContext().getResources().getDimensionPixelSize(R.dimen.crop_height_horizontal_wheel_progress_line);
        mProgressLineMargin = getContext().getResources().getDimensionPixelSize(R.dimen.crop_margin_horizontal_wheel_progress_line);
